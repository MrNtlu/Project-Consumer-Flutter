...
class _AnimeDetailsPage extends StatefulWidget {
  final String id;

  const _AnimeDetailsPage(this.id, {super.key});

  @override
  State<_AnimeDetailsPage> createState() => _AnimeDetailsPageState();
}

class _AnimeDetailsPageState extends State<_AnimeDetailsPage> {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
        create: (_) => _provider,
        child:
            Consumer<AnimeDetailsProvider>(builder: (context, provider, child) {
                    ...
                    if (item != null && item.consumeLater != null) {
                      ...
                      provider
                          .createConsumeLaterObject(ConsumeLaterBody(
                              item.id, item.malID.toString(), "anime"))
                          .then((response) {
                        if (response.error != null) {
                          showCupertinoDialog(
                            context: context,
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            AnimeDetailsInfoColumn(
                                ...
                                "${item.aired.from != null && item.aired.from != "" ? DateTime.tryParse(item.aired.from!)?.dateToHumanDate() : '?'} to ${item.aired.to != null && item.aired.to != "" ? DateTime.tryParse(item.aired.to!)?.dateToHumanDate() : '?'}",
                                item.source,
                                item.type),
                          ],
                        ),
                ...
                if (item.recommendations.isNotEmpty)
                  SizedBox(
                    ...
                    child: DetailsRecommendationList(
                      ...
                      return _AnimeDetailsPage(
                          item.recommendations[index].malId.toString());
                    }),
                  ),
