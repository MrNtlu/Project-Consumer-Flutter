...
class AnimeDiscoverSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final genreList = DiscoverSheetList(
      Constants.AnimeGenreList.where(
          (element) => element.name == provider.genre).firstOrNull?.name,
      genres,
    );

    final demographicList = DiscoverSheetList(
        Constants.AnimeDemographicsList.where(
                (element) => element.name == provider.demographics)
            .firstOrNull
            ?.name,
        Constants.AnimeDemographicsList.map((e) => e.name).toList());

    final themeList = DiscoverSheetList(
        Constants.AnimeThemeList.where(
            (element) => element.name == provider.themes).firstOrNull?.name,
        Constants.AnimeThemeList.map((e) => e.name).toList());

    final statusList = DiscoverSheetList(
        Constants.AnimeStatusRequests.where(
            (element) => element.request == provider.status).firstOrNull?.name,
        Constants.AnimeStatusRequests.map((e) => e.name).toList());

    final seasonList = DiscoverSheetList(
        Constants.AnimeSeasonList.where(
            (element) => element.request == provider.season).firstOrNull?.name,
    ...
    final yearDiscoverList = DiscoverSheetList(
        yearList
            .where((element) => element == provider.year)
            .firstOrNull
            ?.toString(),
    ...
    final streamingPlatformList = DiscoverSheetImageList(
      Constants.AnimeStreamingPlatformList.where(
          (element) => element.request == provider.streaming).firstOrNull?.name,
    ...
    final studiosList = DiscoverSheetImageList(
      Constants.AnimePopularStudiosList.where(
          (element) => element.request == provider.studios).firstOrNull?.name,
                ...
                children: [
                  ...
                  CupertinoButton.filled(
                    onPressed: () {
                      ...
                      final newGenre = Constants.AnimeGenreList.where(
                        (element) =>
                            element.name ==
                            genreList.selectedValueNotifier.value,
                      ).firstOrNull?.name;

                      final newDemographics =
                          Constants.AnimeDemographicsList.where(
                        (element) =>
                            element.name ==
                            demographicList.selectedValueNotifier.value,
                      ).firstOrNull?.name;

                      final newThemes = Constants.AnimeThemeList.where(
                        (element) =>
                            element.name ==
                            themeList.selectedValueNotifier.value,
                      ).firstOrNull?.name;

                      final newStatus = Constants.AnimeStatusRequests.where(
                        (element) =>
                            element.name ==
                            statusList.selectedValueNotifier.value,
                      ).firstOrNull?.request;

                      final newSeason = Constants.AnimeSeasonList.where(
                        (element) =>
                            element.name ==
                            seasonList.selectedValueNotifier.value,
                      ).firstOrNull?.request;

                      final newYear = yearList
                          .where(
                            (element) =>
                                element.toString() ==
                                yearDiscoverList.selectedValueNotifier.value
                                    ?.toString(),
                          )
                          .firstOrNull;
                      ...
                      final newStudio = Constants.AnimePopularStudiosList.where(
                  ...
                  children: [
                    ...
                    ChangeNotifierProvider(
                      ...
                      child: DiscoverSheetImageFilterBody(
                          "Streaming Platforms", streamingPlatformList),
                    ),
                    ChangeNotifierProvider(
                        create: (_) => StreamingPlatformStateProvider(),
                        child: DiscoverSheetImageFilterBody(
